...
function IsThrillerDevPanel:positionTitleButtons()
    if not self.closeButton or not self.collapseButton then return end
    local buttonW = self.closeButton:getWidth()
    self.closeButton:setX(self.width - buttonW - 1)
    self.collapseButton:setX(self.width - buttonW * 2 - 2)
end
...
function IsThrillerDevPanel:createChildren()
    ...
    -- GPTNote: Put the explicit summary-collapse arrow beside the close button at the top-right.
    ...
    self.collapseButton:setImage(nil)
    self.collapseButton:setTitle("↑")
    self.collapseButton.target = self
    self.collapseButton.onclick = IsThrillerDevPanel.onToggleCompact
...
function IsThrillerDevPanel:onToggleCompact()
    ...
    self.collapseButton:setTitle(self.compact and "↓" or "↑")
